    ...
    $.ajax({
        ...
        success: function (resp) {
            ...
            if (data[0] != null) {
                ...
                $perfil = data[0].tipo;
                if($perfil == "admin") {
                    $('#form-oculto').html('<form action="/ParqueaFacil/" name="form1" method="post" style="display:none;"><input type="hidden" name="perfil" value="' + $perfil + '" /><input type="hidden" name="nombres-login" value="' + data[0].nombres + '" /><input type="hidden" name="cargo-login" value="' + data[0].cargo + '" /></form>');
                } else if($perfil == "vigi") {
                    $('#form-oculto').html('<form action="/ParqueaFacil/" name="form1" method="post" style="display:none;"><input type="hidden" name="perfil" value="' + $perfil + '" /><input type="hidden" name="documentovig" value="' + data[0].documento + '" /></form>');
                }
...
function revisa_comprobar(registro) {
    $.ajax({
        ...
        success: function (resp) {
            ...
            if (data[0] != null) {
                $perfil = "prop";
                $('#form-oculto').html(`<form action="/ParqueaFacil/" name="form1" method="post" style="display:none;">
